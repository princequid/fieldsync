...
const COLUMNS = ["Job", "Client", "Technician", "Status", "Priority", "Updated", "Actions"];
...
export default function Table({
  ...
  if (rows.length === 0) {
    return (
      <p className="px-6 py-12 text-center text-[13px] text-[#94A3B8]">
        No jobs to display.
      </p>
    );
  }
          ...
          <thead>
            <tr style={{ background: "#F8FAFC", borderBottom: "1px solid #F1F5F9" }}>
              {COLUMNS.map((col) => (
                <th
                  key={col}
                  className="group/col cursor-default select-none px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-widest text-[#64748B]"
                >
            ...
            {rows.map((job) => {
              ...
              return (
                ...
                <tr
                  ...
                  className="fs-table-row group h-13 cursor-pointer"
                  style={{ borderBottom: "1px solid #F8FAFC" }}
                >
                  {/* Job column: title + number/location */}
                  <td className="px-4 py-0 align-middle">
                    <p className="font-mono text-[10px] text-[#94A3B8]">{job.jobNumber}</p>
                    <p className="mt-0.5 text-[13px] font-medium text-[#0F172A]">{job.title}</p>
                    <p className="mt-0.5 font-mono text-[10px] text-[#94A3B8]">{job.location}</p>
                  </td>

                  {/* Client */}
                  <td className="px-4 py-0 align-middle text-[13px] text-[#374151]">
                    {client?.name ?? "—"}
                  </td>
                  ...
                  >
                    {technician ? (
                      <div className="inline-flex items-center gap-2">
                        ...
                        <span className="text-[13px] text-[#374151]">{technician.name}</span>
                      </div>
                    ) : (
                  ...
                  {/* Updated */}
                  <td className="px-4 py-0 align-middle font-mono text-[11px] text-[#94A3B8]">
                  ...
                  >
                    <div className="flex items-center gap-1.5 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                      ...
                      <button
                        type="button"
                        onClick={() => navigate(`/admin/jobs/${job.id}`)}
                        className="flex h-7 items-center rounded-badge px-3 text-[11px] font-medium text-[#374151]"
